...
LastEditTime: 2023-06-26 20:06:02
...
def train_with_pruning(model, dataloader, criterion, optimizer, device='cpu', num_epochs=10,pruning_rate=0.5):
    ...
    for epoch in range(num_epochs):
        ...
        # 在每个epoch 之后进行剪枝
        prune_model(model, pruning_rate,method='global') # 在每次反向传播之后将模型权重置为零
    return model

net = Net()
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(net.parameters(), lr=1e-3)
net = train_with_pruning(net, train_loader, criterion, optimizer,device='cpu', num_epochs=2,pruning_rate=0.1)

# save model
torch.save(net.state_dict(),'net_train_with_pruning.pth')
